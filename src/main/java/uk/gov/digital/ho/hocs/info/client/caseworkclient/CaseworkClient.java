...
public class CaseworkClient {
    ...
    public String getStageTypeFromStage(UUID caseUUID, UUID stageUUID) {
        ResponseEntity<String> response = restHelper.get(serviceBaseURL,
            String.format("/case/%s/stage/%s/type", caseUUID, stageUUID), String.class);

        if (response.getStatusCodeValue() == 200) {
            log.info("Got Type for stage: {}", stageUUID);
            return response.getBody();
        } else {
            throw new ApplicationExceptions.EntityNotFoundException("Could not get Type for stage %s; response: %s",
                stageUUID, response.getStatusCodeValue());
        }
    }
    ...
    public Set getCasesForUser(UUID userUUID, UUID teamUUID) {
        ResponseEntity<Set> response = restHelper.get(serviceBaseURL,
            String.format("/stage/team/%s/user/%s", teamUUID, userUUID), Set.class);
        if (response.getStatusCodeValue() == 200) {
            log.info("Got cases for User: {}", userUUID);
            return response.getBody();
        } else {
            throw new ApplicationExceptions.EntityNotFoundException("Could not get cases for User %s", userUUID,
                response.getStatusCodeValue());
        }
    }
    ...
    public void clearCachedTemplateForCaseType(String caseType) {
        ResponseEntity<String> response = restHelper.post(serviceBaseURL,
            String.format("/caseType/%s/clearCachedTemplate", caseType), null, String.class);
        if (response.getStatusCodeValue() == 200) {
