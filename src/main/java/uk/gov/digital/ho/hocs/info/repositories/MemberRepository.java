...
//    Set<Member> findAllActiveMembersForCaseType(String caseType);
    ...
    @Query(value = "SELECT m.* FROM member m JOIN member_case_type mct ON m.uuid = mct.member_uuid WHERE mct.case_type = ?1 AND m.active = TRUE", nativeQuery = true)
    Set<Member> findAllActiveMembersForCaseType(String caseType);

    @Query(value = "SELECT m.* FROM member m WHERE m.active = TRUE", nativeQuery = true)
