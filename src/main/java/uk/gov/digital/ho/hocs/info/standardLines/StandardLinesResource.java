...
public class StandardLinesResource {
    ...
    @GetMapping(value = "/casetype/{caseType}/topic/{topicUUID}", produces = APPLICATION_JSON_UTF8_VALUE)
    public ResponseEntity<GetStandardLinesResponse> getTemplateForCaseTypes(@PathVariable String caseType, @PathVariable UUID topicUUID) {
        try {
            StandardLines standardLines = standardLinesService.getStandardLines(caseType, topicUUID);
