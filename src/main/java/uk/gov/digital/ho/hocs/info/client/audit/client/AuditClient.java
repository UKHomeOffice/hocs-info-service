...
public class AuditClient {
    ...
    public void addUsersToTeamAudit(String userUUIDs, Team team) {
        String auditPayload = Json.createObjectBuilder().add(TEAM_UUID, team.getUuid().toString()).add("userUUIDs",
            userUUIDs).build().toString();
    ...
    public void moveToNewUnitAudit(String teamUUID, String oldUnitUUID, String newUnitUUID) {
        String auditPayload = Json.createObjectBuilder().add(TEAM_UUID, teamUUID).add("oldUnit", oldUnitUUID).add(
            "newUnit", newUnitUUID).build().toString();
    ...
    public void updateTeamPermissionsAudit(Set<PermissionDto> permissions) {
        for (PermissionDto permission : permissions) {
            permissionArray.add(
                Json.createObjectBuilder().add(CASE_TYPE, permission.getCaseTypeCode()).add("accessLevel",
                    permission.getAccessLevel().toString()).build());
        }
    ...
    public void deleteTeamPermissionsAudit(Set<PermissionDto> permissions) {
        ...
        String auditPayload = Json.createObjectBuilder().add("permissions", permissionArray).build().toString();
        CreateAuditRequest request = generateAuditRequest(auditPayload,
            EventType.REMOVE_PERMISSIONS_FROM_TEAM.toString());
        sendAuditMessage(request);
    }

    public void removeUserFromTeamAudit(UUID userUUID, UUID teamUUID) {
        String auditPayload = Json.createObjectBuilder().add(TEAM_UUID, teamUUID.toString()).add("userUUID",
            userUUID.toString()).build().toString();
    ...
    public void setTeamActivationFlag(Team team) {
        String auditPayload = Json.createObjectBuilder().add(ACTIVE, team.isActive()).add(TEAM_UUID,
            team.getUuid().toString()).build().toString();
    ...
    public void createCorrespondentType(CorrespondentType correspondentType) {
        String auditPayload = Json.createObjectBuilder().add("correspondentTypeUUID",
            correspondentType.getUuid().toString()).add(DISPLAY_NAME, correspondentType.getDisplayName()).add("type",
            correspondentType.getType()).build().toString();
