...
public interface DocumentTagRepository extends CrudRepository<DocumentTag, String> {
    ...
    @Query(value = "SELECT dt.* FROM document_tag dt INNER JOIN case_type ct ON ct.uuid = dt.case_type_uuid WHERE ct.type = ?1 ORDER BY dt.sort_order", nativeQuery = true)
