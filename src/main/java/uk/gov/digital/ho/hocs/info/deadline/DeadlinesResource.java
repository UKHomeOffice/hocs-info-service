...
public class DeadlinesResource {
    ...
    @GetMapping(value = "/casetype/{caseType}/stagetype/{stageType}/deadline/{received}", produces = APPLICATION_JSON_UTF8_VALUE)
    public ResponseEntity<Deadline> getDeadlineByStage(@PathVariable String caseType, @PathVariable String stageType, @PathVariable String received) {
        try {
            LocalDate receivedDate = LocalDate.parse(received);

            Deadline deadline = deadlinesService.getDeadlineForStage(caseType, stageType, receivedDate);
