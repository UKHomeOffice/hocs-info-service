...
public class UnitService {
    ...
    public Set<UnitDto> getAllUnits() {
        return unitRepository.findAll().stream().map(UnitDto::fromWithoutTeams).collect(Collectors.toSet());
    }
    ...
    public void deleteUnit(UUID unitUUID) {
        ...
        if (teams.isEmpty()) {
            ...
            throw new ApplicationExceptions.UnitDeleteException(
                "Unable to delete Unit {}, active teams are allocated to unit", unitUUID);
        }
