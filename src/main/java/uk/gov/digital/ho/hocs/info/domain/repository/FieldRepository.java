...
public interface FieldRepository extends CrudRepository<Field, String> {
    ...
    @Query(value = "SELECT * from info.field f LEFT JOIN info.field_screen fs ON f.uuid = fs.field_uuid LEFT JOIN info.screen_schema ss ON ss.uuid = fs.schema_uuid WHERE ss.type = ?1 ORDER BY sort_order ASC", nativeQuery = true)
