...
public interface FieldRepository extends CrudRepository<Field, String> {
    ...
    @Query(value = "SELECT * FROM field fi WHERE fi.access_level = ?1 AND fi.uuid IN (SELECT field_uuid FROM field_screen fs INNER JOIN screen_schema ss ON ss.uuid = fs.schema_uuid INNER JOIN case_type_schema cts ON cts.schema_uuid = ss.uuid WHERE fs.field_uuid = fi.uuid AND ss.active = TRUE AND cts.case_type = ?2) ORDER BY fi.id", nativeQuery = true)
    List<Field> findAllByAccessLevelAndCaseType(String accessLevel, String caseType);
}
