...
public class GetMembersResponseTest {
    ...
    public void from() {
        ...
        MemberDto result1 = responseAsList.stream().filter(x -> "Lord Bob".equals(x.getDisplayName())).findAny().orElse(
            null);
        assertThat(result1).isNotNull();
        assertThat(result1.getHouse()).isEqualTo("Lords");
        assertThat(result1.getUuid()).isEqualTo(uuid1);
        MemberDto result2 = responseAsList.stream().filter(x -> "Sir Fred".equals(x.getDisplayName())).findAny().orElse(
            null);
