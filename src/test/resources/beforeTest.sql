delete
from parent_topic
where uuid in ('94a10f9f-a42e-44c0-8ebe-1227cb347f1d',
...
delete
from team_link
where stage_type in ('ST1', 'ST2');

delete
from topic
where uuid in ('11111111-ffff-1111-1111-111111111131',
...
delete
from team_contact
where team_uuid in ('08612f06-bae2-4d2f-90d2-2254a68414b8',
                    '911adabe-5ab7-4470-8395-6b584a61462d',
                    '434a4e33-437f-4e6d-8f04-14ea40fdbfa2');

delete
from correspondent_type
where uuid in ('fe1e8854-72ef-4141-a675-fb06760264fd',
               'c2155e6c-7ecd-450d-86e3-884e48c8c6c7');

delete
from permission
where case_type in ('CT1', 'CT2', 'CT3');

delete
from stage_type
where type in ('ST1', 'ST2');

delete
from team
where unit_uuid in ('09221c48-b916-47df-9aa0-a0194f86f6dd',
...
delete
from case_type
where type in ('CT1', 'CT2', 'CT3');

delete
from unit
where display_name in ('UNIT 2',
                       'UNIT 3',
                       'UNIT 4',
                       'UNIT 5',
                       'UNIT 6');
...
delete
from correspondent_type
where type in ('TEST', 'TEST1', 'TEST2');
...
VALUES ('f62834a0-d231-44c9-bfa1-55dd93fc0aa0', 'Test Case Type 1', 'z9', 'CT1', '09221c48-b916-47df-9aa0-a0194f86f6dd',
        'DISPATCH', true, true),
       ('056cf0eb-becd-49fa-86eb-ba4b7678a515', 'Test Case Type 2', 'za', 'CT2', '09221c48-b916-47df-9aa0-a0194f86f6dd',
        'DISPATCH', true, true),
       ('692283fe-accd-40a5-9b6d-703e1aed7ebd', 'Test Case Type 3', 'zb', 'CT3', '09221c48-b916-47df-9aa0-a0194f86f6dd',
        'DISPATCH', true, true);
